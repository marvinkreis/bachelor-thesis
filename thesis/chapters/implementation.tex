...
Listing~\ref{lst:run_method_excerpt} shows an except of Whisker's run method,
...
% Rather, Whisker waits a configured amount of time after a random input is executed before it chooses the next one.
...
\mnote{TODO: explain that Whisker does Coverage measurement earlier}

Whisker can be used to measure statement coverage.
...
since earlier projects don't save the unique id of each block.
However, this is not a problem since older Scratch programs can simply be converted to Scratch 3.0 projects by
...
\begin{listing}[htpb]
    ...
    \caption{Example of coverage measurement using Whisker}
