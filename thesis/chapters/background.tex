...
which uses Hairball to measure the complexity of uploaded Scratch programs in various categories and scores them accordingly.
...
In order to do this, a dynamic testing approach is more suitable, because tests can simply observe to programs behaviour instead of having to deduct it from the programs source code.
\parspace
...
In order to automate the IO, ITCH replaces these blocks with structures, which give the program configured input text and save the resulting output.
...
% so each test can start from the same state.
\parspace
...
In order to test sprite movements or animations, we need to track information over a period of time.
Hence, it needs to be possible to periodically access sprite attributes (and variables) during the program execution.
\parspace
