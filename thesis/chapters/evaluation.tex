...
\mnote{Plots in one column or two columns?}
...
with a correlation coefficient of $r = 0.893$ for the first run and $r = 0.899$ for the average over 10 runs.
...
with a correlation coefficient of $r = 0.904$ for the first run and $r = 0.905$ for the average over 10 runs.
...
The result indicate a strong correlation with a correlation coefficient of $r = 0.850$ for the first run and $r = 868$ for the average over 10 runs.
...
However, on average only $1.36$ projects per test case and $1.23$ test cases per project show inconsistent results.
This amounts to $4.38\%$ of all test-project pairs, which is still consistent enough for grading purposes.
\parspace
...
The data shows a maximum of $6$ inconsistent projects ($19.35\%$ of projects) per constraint and $5$ inconsistent constraints ($25.00\%$ of constraints) per project.
On average, $1.20$ projects per constraint and $0.77$ constraints per project show inconsistent results,
which amounts to $3.87\%$ of all constraint-project pairs.
\parspace
...
We observed a maximum of $10$ inconsistent projects ($32.23\%$ of projects) per constraint and $6$ inconsistent constraints ($33.33\%$ of constraints) per project.
On average, $4.28$ projects per constraint and $2.48$ constraints per project show inconsistent results,
which amounts to $13.80\%$ of all constraint-project pairs.
...
The number of inconsistencies increased, because the constraints don't take the edges of the screen into account, which prevent the bowl from moving.
...
However, when using randomly generated input, this can happen.
\parspace
...
We repeated this experiment five times and got similar results, with the mean final coverage over ten runs being $95.25\%$.
